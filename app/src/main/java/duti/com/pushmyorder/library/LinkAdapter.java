...
public class LinkAdapter<T> extends RecyclerView.Adapter<LinkAdapter.ViewHolder> {
    ...
    public interface onAdapterItemCheck {
        void onItemChecked(boolean isChecked, Object o, int pos);
    }
    ...
    public void setCheckboxResponseListener(onAdapterItemCheck listener) {
    ...
    public static class ViewHolder extends RecyclerView.ViewHolder {
        ...
        Map<String, EditText> arrEditText = new HashMap<String, EditText>();
        ...
        private void makeViewMap(View view, String sFieldName) {
            if (view instanceof ImageView) {
                ...
                arrEditText.put(sFieldName, (EditText) view);
            } else {
    ...
    public void onBindViewHolder(ViewHolder viewHolder, int position) {
        ...
        for (Map.Entry<String, ImageView> entry : viewHolder.arrImageView.entrySet()) {
            ...
            if (iv != null) {
                if(iv.getTag() != null){
                    if((iv.getTag().toString().contains("Drawable"))){
                        iv.setImageDrawable(dt.tools.getImage(getDrawableImage(data, entry.getKey())));
                    }
                }
            }
        }
            ...
            if (cb != null){
                ...
                cb.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
                    ...
                    public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                        ...
                        if (customListenerCheck != null) customListenerCheck.onItemChecked(isChecked, data, iPos);
                    }
                });
            }
        }

        for (Map.Entry<String, EditText> entry : viewHolder.arrEditText.entrySet()) {
            EditText et = viewHolder.arrEditText.get(entry.getKey());
            if (et != null) {
                if(et.getTag() != null){
                    if((et.getTag().toString().contains("Date"))){
                        et.setText(dt.dateTime.fineFormatDateFromString(getFieldValue(data, entry.getKey())));
                    } else {
                        et.setText(getFieldValue(data, entry.getKey()));
                    }
                } else {
                    et.setText(getFieldValue(data, entry.getKey()));
                }
    ...
    private int getDrawableImage(T obj, String fieldName) {
        int fieldValue = 0;
        Field field = null;
        try {
            field = obj.getClass().getDeclaredField(fieldName);
            field.setAccessible(true);
            if (field.get(obj) != null) fieldValue = (int) field.get(obj);
