...
import static duti.com.pushmyorder.config.Constants.mServerRecordId;
import static duti.com.pushmyorder.config.Constants.mStatus;
import static duti.com.pushmyorder.config.Constants.mSyncedValue;
            ...
            try {
                ...
                if (fieldName.equalsIgnoreCase("$change")) {
                    ...
                    if (!fieldName.equals(primaryKeyField))
                        values.put(fieldName, field.get(item) != null ? field.get(item).toString() : "");
                }
/*                if ((!fieldName.equalsIgnoreCase("$change")) && (!fieldName.equalsIgnoreCase("serialVersionUID")) && (!excludeFields.contains(fieldName))) {
                    if (!fieldName.equals(primaryKeyField))
                        values.put(fieldName, field.get(item) != null ? field.get(item).toString() : "");
                }*/
        ...
        while (cursor.isAfterLast() == false) {
            ...
            for (int i = 0; i < totalColumn; i++) {
                if (cursor.getColumnName(i) != null) {
                    try {
                        //int cursorType = cursor.getType(i);
                        String fieldType = getFieldType(object, cursor.getColumnName(i));
                        switch (fieldType) {
                            case " BLOB":
                                if (cursor.getBlob(i) != null) {
                                    JSONArray jsonArray = new JSONArray(new String(cursor.getBlob(i)));
                                    rowObject.put(cursor.getColumnName(i), jsonArray);
                                } else {
                                    JSONArray jsonArray = new JSONArray(new String(new byte[0]));
                                    rowObject.put(cursor.getColumnName(i), jsonArray);
                                }
                                break;
                            case " INTEGER":
    ...
    /*                                if(cursor.getColumnName(i).equals("FingerImage")){
                                    if(cursor.getBlob(i) != null){
                                        JSONArray test = new JSONArray(new String(cursor.getBlob(i)));
                                        rowObject.put(cursor.getColumnName(i), test);
                                    } else {
                                        JSONArray test = new JSONArray(new String(new byte[0]));
                                        rowObject.put(cursor.getColumnName(i), test);
                                    }
                                } */
    ...
    @Override
    public Object getAllWithPreClause(String preClause, String where, String orderBy, Class<?> cls) {
        SQLiteDatabase db = this.getReadableDatabase();

        String selectQuery = "SELECT " + preClause + " FROM " + mTableName + " " + where + " " + orderBy;
        Cursor cursor = db.rawQuery(selectQuery, null);
...
/*        SQLiteDatabase db;
    ...
    @Override
    public Object getWithTableName(String where, String orderBy, Class<?> cls) {
        SQLiteDatabase db = this.getReadableDatabase();
        if (!TextUtils.isEmpty(tableMap.get(mTableName))) mTableName = tableMap.get(mTableName);
        String selectQuery = "SELECT * FROM " + mTableName + " " + where + " " + orderBy;
        Cursor cursor = db.rawQuery(selectQuery, null);
        if (cursor != null)
            cursor.moveToFirst();
    ...
    public String getFiledValue(String fieldName, String sql)
    {
        ...
        Cursor cursor = db.rawQuery(sql, null);
        if (cursor != null) cursor.moveToFirst();
        if(cursor.getCount()>0) fieldValue = cursor.getString(cursor.getColumnIndex(fieldName));
        cursor.close();
        db.close();
        return fieldValue;
    }

    public long getRecordCount(String sqlLine) {
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = db.rawQuery("select count(*) count from " + mTableName + sqlLine, null);
        if (cursor != null) cursor.moveToFirst();
...
/*        if (isExternalDatabase(mContext)) {
    ...
    @Override
    public void makeReadyForSync(long recordId) {
